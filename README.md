...
- **Вход через Telegram** — кнопка на `/login` генерирует одноразовый токен, открывает бота `@kyanchir_store_bot`, бот привязывает токен к пользователю после получения контакта и фронтенд завершает сессию через провайдера `telegram-credentials`.【F:src/app/(site)/(auth)/login/page.tsx†L1-L260】【F:src/lib/telegram.ts†L1-L420】
