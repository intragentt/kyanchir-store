...
| Конфигурации | ⚠️ Ревизия секретов | `tsconfig.json` исправлен, cron вынесен в инфраструктуру, но отсутствуют `BOT_API_SECRET` и ключи YooKassa — следите за чек-листом безопасности.【F:tsconfig.json†L1-L26】【F:docs/operations/cron.md†L1-L12】【F:docs/SECURITY_AUDIT.md†L17-L31】 |
...
- **Аутентификация**: на `/login` кнопка «Авторизоваться через телеграмм» сразу перенаправляет в бот `@kyanchir_store_bot`, который просит контакт с ссылками на политику/оферту, затем email. Если email свободен — бот создаёт доступ и отправляет кнопку авторизации, если уже зарегистрирован — высылает 6-значный код на почту и ждёт его ввода в чате для объединения учётных записей.【F:src/app/(site)/(auth)/login/page.tsx†L1-L340】【F:src/lib/telegram.ts†L1-L420】
...
- **tsconfig.json**: опечатка устранена, запускайте `tsc --noEmit` перед релизом и добавьте проверку в CI.【F:tsconfig.json†L1-L26】
- **Cron**: расписание синхронизации каталога конфигурируется через Vercel Scheduler. Следуйте инструкции `docs/operations/cron.md`, секреты храните в Vercel Env.【F:docs/operations/cron.md†L1-L12】
- **Security audit**: актуальный список обязательных env-ключей и статус их наличия фиксируется в `docs/SECURITY_AUDIT.md`. Проверяйте перед каждым релизом.【F:docs/SECURITY_AUDIT.md†L4-L33】
...
5. **Безопасность**: перед деплоем проверяйте, что `.env*` и файлы с секретами не попали в git, сверяйтесь с `docs/SECURITY_AUDIT.md`. Используйте Prisma роли/NextAuth для защиты админских функций.【F:src/app/api/admin/products/update-stock/route.ts†L13-L26】【F:docs/SECURITY_AUDIT.md†L4-L33】
...
4. **Секреты и конфиги** — получить `BOT_API_SECRET` и ключи `YOOKASSA_*`, настроить cron через сервисный токен, включить `tsc --noEmit` в CI.【F:docs/SECURITY_AUDIT.md†L17-L31】【F:docs/operations/cron.md†L1-L12】【F:tsconfig.json†L1-L26】
